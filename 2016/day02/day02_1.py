...
def main():
    ...
    for instruction in instructions:
        for direction in list(instruction):
            if direction == 'U' :
                row = clamp(row - 1, low_bound, high_bound)
            elif direction == 'R':
                col = clamp(col + 1, low_bound, high_bound)
            elif direction == 'D':
                row = clamp(row + 1, low_bound, high_bound)
            else:
                col = clamp(col - 1, low_bound, high_bound)
        key_number = (row * 3) + 1 + col
        sequence.append(key_number)

    code = "".join(str(n) for n in sequence)
